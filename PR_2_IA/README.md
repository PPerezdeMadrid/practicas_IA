...
## Funcionamiento
En la clase Graphs.java de los test se encuentra un ejemplo de los gráficos para testear con el algoritmo A* 

He elegido el grafo "Undirected" que se puede ver de esta forma:

![Alt text](defGrafo.png)
![Alt text](relacionesGrafo.png)

Si ejecutamos el algoritmo sobre dicho grafo:
![Alt text](resultadoAestrella.png)

Tras ejecutar el algoritmo quedaría de esta forma:
...
![Alt text](listaABIERTA.png)

### 2) y 3) ¿Que variable representa la función g? ¿ Y la función f?
![Alt text](funcionfyg.png)

### 4) ¿Qué método habría que modificar para que la heurística representarala distancia aérea entre vértices?
![Alt text](heuristica.png)

### 5) ¿Realiza este método reevaluación de nudos cuando se encuentra unanueva ruta a un determinado vértice? Justifique la respuesta.
Sí, lo realiza pero en AStar (por definición del algoritmo)
![Alt text](reevaluacionNudos.png)
Como se observa se hace una clara reevaluación de nudos con las variables "tentativeGScore", además de ordenar la lista al final, en caso de haber elegido otro nudo al reevaluar. 
